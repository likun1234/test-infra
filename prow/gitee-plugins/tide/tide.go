...
func (t tide) checkPrLabel(labels map[string]bool, cfg *pluginConfig, prAuthor string) (bool, string) {
	missing, toRemove := cfg.labelDiff(labels)
	nm := len(missing)
	ne := len(toRemove)
	if nm == 0 && ne == 0 {
		return true, ""
	...
	if nm != 0 && ne != 0 {
		return false, fmt.Sprintf(
			tideNotification+" It needs to add **%s** label%s and remove **%s** label%s to get be merged.",
			prAuthor,
			strings.Join(missing, ", "), s(nm),
			strings.Join(toRemove, ", "), s(ne),
		)
	}

	if nm != 0 {
		return false, fmt.Sprintf(
			tideNotification+" It needs to add **%s** label%s to get be merged.",
			prAuthor,
			strings.Join(missing, ", "), s(nm),
		)
	}
	...
	return false, fmt.Sprintf(
		tideNotification+" It needs to remove **%s** label%s to get be merged.",
		prAuthor,
		strings.Join(toRemove, ", "), s(ne),
	)
}
